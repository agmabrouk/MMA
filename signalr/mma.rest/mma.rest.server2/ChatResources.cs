...
namespace mma.rest.server
{
    public class ChatResources : IChatResources
    {
        ...
        public ChatResources()
        {
            ...
            mmaChatRoom generalChatRoom = new mmaChatRoom() { ChatRoomName = GeneralChatRoomName, CreationTime = DateTime.UtcNow, Id = GeneralChatRoomID.ToString(), isActive = true, ChatRoomClients = new List<mmaClient>(), ChatRoomMessages = new List<mmaChatMessage>() };
        ...
        public void UpdateClientSyncDateTimeForChatRoom(string? clientActiveChatRoom, string? clientRequesterId)
        {
            ...
            if (c != null)
            {
                lock (_lock)
                {
                    c.LastSyncDateTime = DateTime.UtcNow;
                }
            }
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    mmaChatRoom mmaChatRoom = new mmaChatRoom
                    {
                        ...
                        isActive = true,
                        ChatRoomMessages = new List<mmaChatMessage>(),
                        ChatRoomClients = new List<mmaClient>()
                    };
        ...
        public List<string>? removeClientFromAllChatRooms(string rClientId)
        {
            try
            {
                ...
                if (rClientId is null) return null;
                    ...
                    foreach (var cr in targetChatRooms)
                    {
                        if (cr.Value != null)
                            foreach (var c in cr.Value.ChatRoomClients.Where(c => c.ClientId.Equals(rClientId)))
                            {
                                ...
                                chatRoomsNames.Add(cr.Value.ChatRoomName);
    ...
    public interface IChatResources
    {
        ...
        List<string> removeClientFromAllChatRooms(string rClientId);
